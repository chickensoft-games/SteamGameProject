...
Internal package for testing CI/CD workflows.

---

<p align="center">
<img alt="Cardboard Box with Chickensoft Logo" src="SteamGameProject/icon.png" width="200">
</p>
...
dotnet new --install SteamGameProject
...
You can debug the included test project for your package in `SteamGameProject/` by opening the root of this repository in VSCode and selecting one of the launch configurations: `Debug Tests` or `Debug Current Test`.
...
By default, a test project in `SteamGameProject/` is created for you to write tests for your package. [GoDotTest] is already included and setup, allowing you to focus on development and testing.
...
Tests are executed by running the Godot test project in `SteamGameProject` from the command line and passing in the relevant arguments to Godot so that [GoDotTest] can discover and run tests.
...
The included workflow in [`.github/workflows/version_change.yaml`](.github/workflows/version_change.yaml) can be manually dispatched to open a pull request that replaces the version number in `SteamGameProject/SteamGameProject.csproj` with the version you specify in the workflow's inputs.
...
[line-coverage]: SteamGameProject/badges/line_coverage.svg
[branch-coverage]: SteamGameProject/badges/branch_coverage.svg
